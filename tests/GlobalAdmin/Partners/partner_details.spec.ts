...
test('Global Admin - Partners - Partner Details', { tag: ["@global_admin", "@partners", "@partner_details"] }, async ({ page }) => {
...
await allure.step("Click partner from the list : ", async() =>{ await page.locator('[data-testid="item-row-ok1"]').click();});

await allure.step("Verify correct Name : OK Info", async() =>{ await expect(page.locator('//div[text()="Name"]/following-sibling::div')).toHaveText('OK Info');});

await allure.step("Verify correct System Id : ok1", async() =>{ await expect(page.locator('//div[text()="System Id"]/following-sibling::div')).toHaveText('ok1');});

await allure.step("Verify correct partner users : 5", async() =>{ await expect(page.locator('//div[text()="Partner Users"]/following-sibling::div')).toHaveText('5');});

await allure.step("Verify correct customer users : 1", async() =>{ await expect(page.locator('//div[text()="Customer Users"]/following-sibling::div')).toHaveText('1');});
