...
test('Global Admin - Services - Service List - CSV (Default)', { tag: ["@global_admin", "@services", "@service_list_csv_default"] }, async ({ page }) => {
...
await allure.step("Verify downloaded file : ", async() =>{ await expect(await CSVReader.readMostRecentFile("Downloads")).not.toBe("");});

await allure.step("Verify downloaded file : ", async() =>{ await expect(await CSVReader.readMostRecentFile("Downloads")).not.toBe(null);});
