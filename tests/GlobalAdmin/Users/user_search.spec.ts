...
test('Global Admin - Users - Search', { tag: ["@global_admin", "@users", "@user_search"] }, async ({ page }) => {
...
await allure.step("Verify search result : ", async() =>{ await expect(page.locator('//tr//div[text()="Rallen Estrella"]')).toBeVisible();});

await allure.step("Enter email address : rallen.estrella@istar-x.com", async() =>{ await page.locator('[placeholder="Search Users"]').fill('rallen.estrella@istar-x.com');});

await allure.step("Verify search result : ", async() =>{ await expect(page.locator('//tr//div[text()="rallen.estrella@istar-x.com"]')).toBeVisible();});
