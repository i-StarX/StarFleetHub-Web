...
test('Global Admin - Users - User Details', { tag: ["@global_admin", "@users", "@user_details"] }, async ({ page }) => {
...
await allure.step("Enter Name : Rallen Estrella", async() =>{ await page.locator('[placeholder="Search Users"]').fill('Rallen Estrella');});

await allure.step("Click Search result : ", async() =>{ await page.locator('//tr//div[text()="Rallen Estrella"]').click();});

await allure.step("Verify correct name : Rallen Estrella", async() =>{ await expect(page.locator('//div[text()="Name"]/following-sibling::div')).toHaveText('Rallen Estrella');});

await allure.step("Verify correct role : admin", async() =>{ await expect(page.locator('//div[text()="Role"]/following-sibling::div')).toHaveText('admin');});

await allure.step("Verify correct email : rallen.estrella@istar-x.com", async() =>{ await expect(page.locator('//div[text()="Email"]/following-sibling::div')).toHaveText('rallen.estrella@istar-x.com');});
...
await allure.step("Verify correct partner name : None", async() =>{ await expect(page.locator('//div[text()="Partner Name"]/following-sibling::div')).toHaveText('None');});

await allure.step("Verify correct customer name : None", async() =>{ await expect(page.locator('//div[text()="Customer Name"]/following-sibling::div')).toHaveText('None');});

await allure.step("Verify correct customer account number : None", async() =>{ await expect(page.locator('//div[text()="Customer account number"]/following-sibling::div')).toHaveText('None');});
