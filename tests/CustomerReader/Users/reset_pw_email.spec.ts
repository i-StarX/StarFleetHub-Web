...
test('Customer Read Only - Users - Reset Password email', { tag: ["@customer_reader", "@users", "@customer_reset_pw_email"] }, async ({ page }) => {
...
await allure.step("Click Reset Password button : ", async() =>{ await page.getByRole('button', {name : 'Reset Password', exact: true}).click();});
