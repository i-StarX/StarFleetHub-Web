...
test('Customer Read Only - Analytics - Partner Details', { tag: ["@customer_reader", "@analytics", "@customer_partner_details"] }, async ({ page }) => {
...
await allure.step("Verify correct Partner Name : TPZ-Ops", async() =>{ await expect(page.locator('//h2[text()="Partner Details"]/ancestor::div[@data-testid="tile-content"]//div[text()="Name"]/following-sibling::div')).toHaveText('TPZ-Ops');});

await allure.step("Verify correct System Id : tpzdemo", async() =>{ await expect(page.locator('//h2[text()="Partner Details"]/ancestor::div[@data-testid="tile-content"]//div[text()="System Id"]/following-sibling::div')).toHaveText('tpzdemo');});
