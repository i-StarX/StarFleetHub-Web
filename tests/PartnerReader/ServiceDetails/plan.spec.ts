...
test('Partner Read Only - Service Details - Plan', { tag: ["@partner_reader", "@service_details", "@pr_plan"] }, async ({ page }) => {
...
await allure.step("Verify correct plan currency code : ARS", async() =>{ await expect(page.locator('//div[text()="Plan Currency Code"]/following-sibling::div')).toHaveText('ARS');});
