...
test('Partner Read Only - Users - Search', { tag: ["@partner_reader", "@users", "@pr_user_search"] }, async ({ page }) => {
...
await allure.step("Verify search result : ", async() =>{ await expect(page.locator('//tr//div[text()="Rallen Customer Reader"]')).toBeVisible();});

await allure.step("Enter email address : rallen.estrella+customerreader@istar-x.com", async() =>{ await page.locator('[placeholder="Search Users"]').fill('rallen.estrella+customerreader@istar-x.com');});

await allure.step("Verify search result : ", async() =>{ await expect(page.locator('//tr//div[text()="rallen.estrella+customerreader@istar-x.com"]')).toBeVisible();});
