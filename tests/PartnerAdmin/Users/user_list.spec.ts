...
test('Partner Admin - Users - User List', { tag: ["@partner_admin", "@users", "@pa_user_list"] }, async ({ page }) => {
...
await allure.step("Verify partner column is visible : ", async() =>{ await expect(page.locator('//tr[@data-testid="item-list"]/th//div[text()="partner"]')).toBeVisible();});

await allure.step("Verify customer column is visible : ", async() =>{ await expect(page.locator('//tr[@data-testid="item-list"]/th//div[text()="customer"]')).toBeVisible();});

await allure.step("Verify role column is visible : ", async() =>{ await expect(page.locator('//tr[@data-testid="item-list"]/th//div[text()="role"]')).toBeVisible();});
