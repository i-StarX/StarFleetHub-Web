...
test('Partner Admin - Users - Add Partner User', { tag: ["@partner_admin", "@users", "@pa_add_partner_user"] }, async ({ page }) => {
...
await allure.step("Enter Email address : rallen.estrella+tc03@istar-x.com", async() =>{ await page.locator('#email').fill('rallen.estrella+tc03@istar-x.com');});

await allure.step("Enter mobile : +15551231234", async() =>{ await page.locator('#mobile').fill('+15551231234');});

await allure.step("Select role : Partner Reader", async() =>{ await page.locator('[data-testid="allocation-role-selector"]').selectOption('Partner Reader');});
